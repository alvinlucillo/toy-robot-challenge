...
func TestProcess(t *testing.T) {
	testCases := map[string]struct {
		...
		"successful process - 4": {[]string{"PLACE 3,3,WEST", "MOVE", "RIGHT", "MOVE", "LEFT", "REPORT"}, []string{MessageNotMovedOutOfBounds, "> Output: 2,3,WEST"},
			ro.RobotState{
				X:         2,
				Y:         3,
				Direction: ro.DirectionWest,
		...
		"failed process - place args out of bounds": {[]string{"PLACE -1,3,WEST"}, []string{MessageNotPlacedOutOfBounds},
			ro.RobotState{
				X:         -1,
				Y:         -1,
				Direction: -1,
		...
		"failed process - move out of bounds": {[]string{"PLACE 3,3,EAST", "MOVE"}, []string{MessageNotMovedOutOfBounds},
			ro.RobotState{
				X:         3,
				Y:         3,
